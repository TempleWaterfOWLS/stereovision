...
main( int argc, char** argv )
{
    ...
    switch ( argc )
    {
        case 4:
            szCalFile	    = argv[1];
            szInputFile	    = argv[2];
            szDisparityBase = argv[3];
            break;

		default:
		  // use defaults - no arguments
		  printf( "Usage: %s <cal file> <input file> "
				  "<output disparity basename>\n", argv[0] );
		  printf( "Using default arguments: customizedstereo %s %s %s\n\n",
				  szCalFile.c_str(),
				  szInputFile.c_str(),
				  szDisparityBase.c_str() );
		  break;
    }
